...
If there is a wall, we get a 'reward' of $-100$ and the game ends $r_1 = -100$.
If there is nothing, we get a reward of $-1$, $r_1 = -1$, and if we got to the end, we get a reward of $+100$ and the game ends $r_1 = 100$.
...
The goal is to finish the game with the maximum (discounted) reward possible $R = \sum_{t=1}^{+\infty} \gamma^t r_t$.
